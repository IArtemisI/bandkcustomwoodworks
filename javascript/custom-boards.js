...
document.addEventListener("DOMContentLoaded", () => {
   ...
   const populateReview = () => {
      ...
      if (reviewContactEl) {
         reviewContactEl.textContent = `${getValue("name")} • ${getValue("email")} • ${getValue("phone")}`;
      }
      
      // Size
      const reviewSizeEl = $("#review-size");
      if (reviewSizeEl) {
         reviewSizeEl.textContent = `${getValue("length")} × ${getValue("width")} × ${getValue("height")}`;
      }
      ...
      if (patternsContainer) {
         ...
         if (!patternsChecked.length && !otherPattern) patternsContainer.textContent = "—";
      }
      ...
      if (reviewEngravingEl) reviewEngravingEl.textContent = getValue("engraving");
      
      // Files (from hidden field)
      const filesRaw = $('[name="engraving_files_names"]')?.value?.trim() || "";
      const reviewFilesEl = $("#review-engraving-files");
      if (reviewFilesEl) reviewFilesEl.textContent = filesRaw ? `Files: ${filesRaw}` : "Files: —";
   };
   ...
   const showStep = (step) => {
      ...
      slides.forEach((slide) => slide.classList.toggle("active", Number(slide.dataset.step) === step));
   ...
   tabs.forEach((tab) => {
      tab.addEventListener("click", () => {
         ...
         if (currentStep === 1 && targetStep > 1 && !validateStep1(true)) {
            const firstBad = [nameInput, emailInput, phoneInput].find((f) => f && !f.checkValidity());
            firstBad?.focus();
            return;
         }
   ...
   ========================================================= */
   const fileInputs = [$("#engraving_file_1"), $("#engraving_file_2"), $("#engraving_file_3")].filter(Boolean);
   ...
   const updateFileNames = () => {
      ...
      .map((input) => (input.files && input.files[0] ? input.files[0].name : ""))
      .filter(Boolean);
      
      if (fileNamesEl) fileNamesEl.textContent = names.length ? `Selected: ${names.join(", ")}` : "";
      if (fileNamesHidden) fileNamesHidden.value = names.join(", ");
   };
   
   fileInputs.forEach((input) => input.addEventListener("change", updateFileNames));
   
   // Init
   showStep(1);
   window.addEventListener("resize", () => requestAnimationFrame(moveUnderline), { passive: true });
});